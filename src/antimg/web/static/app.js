...
async function renderHedged(d) {
  ...
  if (s.scalp_model === "grid") {
    ...
    const tf = s.grid_timeframe || "weekly";
    verdict += `СКАЛЬП-МОДЕЛЬ: grid (событийная сетка, дневной такт) · шаг от ${tf}-ATR — ${s.scalp_round_trips} круговых (~${rtYr}/год).\n`
      + (tf !== "daily"
          ? `  📐 Шаг сетки от ${tf}-ATR (шире дневного диапазона) ⇒ дневной бар = «интрадей» внутри ${tf}-осцилляции;\n`
            + `     многодневные круговые сделки теперь честно видны на дневках (твоя идея — режим «раз в день»).\n`
          : `  📐 Шаг от ДНЕВНОГО ATR (тесная сетка) — для неё внутридневной инфы нет; попробуй weekly/monthly TF.\n`)
      + `ЧТО ДНЕВНОЙ БЭКТЕСТ МЕРЯЕТ ЧЕСТНО:\n`
      + `  ✅ ТЕТА — точно (длинные опционы DTE ${$("#form-hedged [name=dte_days]").value}д = медленная тета, BS-MtM). Твоя идея сработала: бид теты схлопнулся.\n`
      + `  ✅ ГАММА стреддла на крупных движениях — точно (это и есть «большая рыба» доктрины).\n`
      + `  ⚠ СКАЛЬП — НИЖНЯЯ ОЦЕНКА, НЕ полный: дневной бар содержит ~${rtYr} круговых/год, а живой ПИ\n`
      + `    делает ~200–250/МЕСЯЦ (~2500/год) на интрадей-осцилляциях, которых в дневном баре ПРОСТО НЕТ.\n`
