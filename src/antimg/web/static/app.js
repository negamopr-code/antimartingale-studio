...
async function renderHedged(d) {
  ...
  if (s.scalp_model === "grid") {
    ...
    verdict += `СКАЛЬП-МОДЕЛЬ: grid · шаг от ${tf}-ATR×${$("#form-hedged [name=grid_atr_frac]").value} · ре-центр ${rc>0?`каждые ${rc}д`:"OFF (заморожен)"} — ${s.scalp_round_trips} круговых (~${rtYr}/год).\n`
      + (rc > 0
          ? `  📐 Сетка СЛЕДУЕТ за ценой (ре-центрируется каждые ${rc}д) — скальпит ТЕКУЩИЙ диапазон, а не годовалый страйк. Это убирает «замороженную» утечку.\n`
          : `  ⚠ Сетка ЗАМОРОЖЕНА на страйке — перестаёт скальпить, когда цена уходит в тренд. Поставь Re-center ~21.\n`)
