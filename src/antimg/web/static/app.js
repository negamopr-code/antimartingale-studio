...
async function renderHedged(d) {
  ...
  if (s.scalp_model === "grid") {
    ...
    verdict += `СКАЛЬП-МОДЕЛЬ: grid · шаг от ${tf}-ATR×${$("#form-hedged [name=grid_atr_frac]").value} · ре-центр ${rc>0?`каждые ${rc}д`:"OFF (заморожен)"} — ${s.scalp_round_trips} круговых (~${rtYr}/год).\n`
      ...
      + `  ⚠ СКАЛЬП — всё равно НИЖНЯЯ ОЦЕНКА: даже со следящей сеткой дневной бар даёт лишь ~${rtYr} круговых/год.\n`
      + `    Живой ПИ делает ~2500/год на ТАЙ-В-ДЕНЬ осцилляциях — они МЕНЬШЕ дневного ATR и в баре их НЕТ.\n`
      + `    Крупные (≥2·ATR) развороты, что дневной бар ВИДИТ, редки (~5/год) и часто НЕ разворачиваются (тренд).\n`
      + `    ⇒ бэктест меряет в основном ЛОНГ-ВОЛ СТРЕДДЛ (ядро), а НЕ скальп, который в боковике кормит тету.\n`
      + `  Итог: на этом инструменте ${s.straddle_pnl >= 0 ? "стреддл уже несёт" : "стреддл сам по себе в минусе"} `
      + `(${f(s.straddle_pnl)}); скальп тут ${f(s.scalp_pnl)} — но его реальный вклад в боковике НЕ виден на дневках.\n`
      + `  Для честного скальпа нужны ВНУТРИДНЕВНЫЕ данные. Дневной бэктест = справедливый тест «купи длинный\n`
      + `  стреддл и ролль» (он работает на волатильных/трендовых: крипта/металлы), но НЕ тест скальпинга.`;
  }
  else
...
function renderHiRules(d, s) {
  ...
  const R = [
    ...
    ["part", "Правило трёх третей (⅓/⅓/⅓)", "скальп-лимит = ⅓ фьючей ✓; тр−резерв = ГАММА стреддла (а не отдельная не-хеджир. треть) ⚠"],
    ["ok", "Контр-трендовый скальпинг", `контр-входов: ${s.scalp_opens}`],
    ["ok", "Экспоненциальная сетка частей", "уровни на смещениях шаг·m^k — дальние части дальше (не вываливаем у центра)"],
    ["ok", "Ответные заявки (книжим возврат)", `круговых сделок: ${s.scalp_round_trips}`],
    [s.confident_flat_days > 0 ? "part" : "part", "Уверенный флет → наращивать лот", `детектирую (${s.confident_flat_days} дн ≥3 циклов), но ЛОТ ПОКА НЕ наращиваю ⚠`],
    ...
    [bb ? "ok" : "no", "Не фейдить тренд — встать в сторону (BB)", bb ? `вне полосы Боллинджера ${s.trend_days} дн → новые контр-входы СТОП` : "BB-гейт ВЫКЛЮЧЕН — фейдим каждый уровень ❌"],
    ["part", "Роллирование только на крупном ходе И в плюсе", "роллю ПО РАСПИСАНИЮ (у экспирации), НЕ по условию «ход ≥ цены колла + плюс-зона» ⚠"],
    ...
    ["part", "Скальп на дневных барах", "дневной бар держит ~1 разворот → интрадей-скальп (~10/день) НЕ виден; нужен внутридневной фид ⚠"],
  ];
  const ic = { ok: "✅", part: "⚠", no: "❌" };
  const col = { ok: "#3fb950", part: "#d29922", no: "#f85149" };
  const rows = R.map(([st, rule, note]) =>
    `<tr><td style="text-align:center;color:${col[st]}">${ic[st]}</td>`
    + `<td style="white-space:nowrap">${rule}</td><td style="color:#8b949e">${note}</td></tr>`).join("");
