...
function renderHiRules(d, s, id) {
  ...
  const R = [
    ...
    [(s.intraday_bars > 0) ? "ok" : "part", "Скальп: внутридневной фид",
      (s.intraday_bars > 0
        ? `✅ ВКЛЮЧЁН ВНУТРИДНЕВНОЙ ФИД: грид прошёл ${s.intraday_bars.toLocaleString()} часовых баров → скальп ловит внутридневной чоп, а не 1 разворот в день. Круговых: ${s.scalp_round_trips}.`
        : "дневной OHLC-бар содержит ~1 разворот; живой интрадей-скальп ~10/день ВНУТРИ дня в баре НЕ виден → скальп недо-измерен. Переключи «Scalp data → hourly» (yfinance ~2 года часовых) — тогда ✅.")],
  ];
  const ic = { ok: "✅", part: "⚠", no: "❌" };
