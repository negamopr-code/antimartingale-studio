...
function renderHiRules(d, s) {
  ...
  const R = [
    ...
    ["part", "Правило трёх третей (⅓/⅓/⅓)",
      "ЕСТЬ эффект: нейтральная база (−1 фьюч), скальп ≤ ⅓ (не голый), тренд-резерв = ГАММА (на росте позиция сама становится long). "
      + "НЕ буква: коллы не дроблю на 3 ЯВНЫЕ трети и не ослабляю базовый хедж до 33%, чтобы активно «отпустить тренд» (его тащит гамма); скальп-лимит = ⅓ фьючей, а в доктрине ⅓ коллов → потому ЧАСТИЧНО."],
    ...
    ["part", "Уверенный флет → наращивать лот",
      `ДЕТЕКТИРУЮ ≥3 чистых цикла подряд (${s.confident_flat_days} дн, зелёный пунктир на графике), НО размер рабочей части на накопленную прибыль ПОКА НЕ увеличиваю → потому ЧАСТИЧНО. Полное = масштабировать part_lots после серии.`],
    ...
    ["part", "Скальп на дневных барах",
      "дневной OHLC-бар содержит ~1 разворот; живой интрадей-скальп ~10/день ВНУТРИ дня в баре НЕ виден → скальп-часть недо-измерена. Это ограничение ДАННЫХ, а не логики (нужен внутридневной фид) → потому ЧАСТИЧНО."],
  ];
  const ic = { ok: "✅", part: "⚠", no: "❌" };
