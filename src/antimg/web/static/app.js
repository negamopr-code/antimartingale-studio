...
async function renderHedged(d) {
  ...
  if (s.scalp_model === "grid")
    verdict += `СКАЛЬП-МОДЕЛЬ: grid (событийная сетка на ДНЕВНОМ такте) — ${s.scalp_round_trips} круговых сделок.\n`
      + `✅ Дневной бар РЕПРЕЗЕНТАТИВЕН: шаг сетки ≈ дневной ATR, ордера исполняются по пути бара (OHLC),\n`
      + `  залипшие части реально маркируются и закрываются на ролле — без фактора-заглушки.\n`
      + `  Длинные опционы (DTE ${$("#form-hedged [name=dte_days]").value}д) = медленная тета ⇒ не нужен быстрый интрадей,\n`
      + `  хватает редких крупных круговых, которые дневной бар видит целиком (мысль из твоего запроса).\n`
      + `  Стреддл (BS-MtM) точен. Контр-трендовый скальп зарабатывает в боковике и слегка теряет в тренде\n`
      + `  (залипшие части) — там за тету платит гамма стреддла. Чем длиннее DTE, тем меньше бьёт тета.`;
  else
    verdict += `СКАЛЬП-МОДЕЛЬ: range (интрадей-эвристика).\n`
      + `⚠ ДНЕВНЫЕ бары видят ~1 разворот в день ((H−L)−|Close−Open|), а реальный интрадей-скальп делает\n`
      + `  ~10 круговых/день на 1-мин. ⇒ ПЕССИМИСТИЧНАЯ НИЖНЯЯ ОЦЕНКА; тета доминирует сильнее, чем в живой ПИ.\n`
      + `  Для честной картины переключись на модель «grid» с длинным DTE (180–365) — дневной бар станет\n`
      + `  репрезентативным. (КПД ${$("#form-hedged [name=scalp_efficiency]").value}, Max RT ${$("#form-hedged [name=max_rt_per_day]").value} влияют только тут.)`;
...
async function renderHiScan(d) {
  ...
  verdict += `\n⚠ Огромный net на крипте — артефакт КОМПАУНДИНГА: премия пересайзится в 20% растущего `
    ...
    + `Доктрина зовёт волатильные инструменты (серебро/ETH) — там гамма стреддла окупает тету, а скальп идёт сверху.`;
  const sm = (d.params || {}).scalp_model;
  if (sm === "grid")
    verdict += `\nСкальп-модель «grid» (событийная сетка, дневной такт) + длинный DTE ${(d.params || {}).dte_days}д ⇒ `
      + `дневной бар репрезентативен (не нижняя оценка); тета медленная. CAGR можно читать прямо.`;
  else
    verdict += `\n⚠ Скальп-модель «range» = пессимистичная нижняя оценка (дневные бары прячут интрадей-сделки). `
      + `Для честной картины запусти bulk на модели «grid» с длинным DTE.`;
